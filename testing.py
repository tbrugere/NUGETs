...
def get_mean_relative_error(model):
    ...
    for batch in tqdm(test):
        ...
        distances = batch.distances.numpy()
        avg_relative_error = (pred - distances)/(distances + 1e-5)
...
for i in range(len(yaml_files)):
    ...
    if not Path(ckpt_dir).exists() or not any(Path(ckpt_dir).glob('*.pt')):
        print("Experiment not finished:", cfg_file)
        continue
    latest_ckpt = max(Path(ckpt_dir).glob("*.pt"), key=lambda f: f.stat().st_mtime)
    ckpt = torch.load(f'workdir/models/{dirname}/{latest_ckpt}')
    model.load_state_dict(ckpt['state_dict'])
    model.eval()
    model.batch_size = 100000
    all_models.append(model)

with open(f'workdir/results/{args.output_file}.csv', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    for i in range(len(yaml_files)):
        name = yaml_files[i].name[:-5]
        model = all_models[i]
        avg, std, all = get_mean_relative_error(model)
        print(name, avg, std)
        writer.writerow([name, all])