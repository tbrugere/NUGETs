...
class MinimumEnclosingAnnulusTask(ShapefittingTask):
    ...
    def compute_metrics(self, datapoint: LabeledSetDatapoint, results: Tensor):
        gt = datapoint.label
        gt_r = datapoint.label[:, -2]
        batch_radius_error = torch.abs(gt_r - results[:, -2])
        radius_relative_error = batch_radius_error/(gt_r + 1e-3)
        inner_radius_mean_relative_error = radius_relative_error.mean()

        gt_r_o = datapoint.label[:, -1]
        batch_radius_error = torch.abs(gt_r_o - results[:, -1])
        radius_relative_error = batch_radius_error/(gt_r_o + 1e-3)
        outer_radius_mean_relative_error = radius_relative_error.mean()

        gt_center = datapoint.label[:, :-2]
        predicted_center = results[:, :-2]
        center_diff = torch.norm(gt_center - predicted_center, dim=1)
        center_diff_mean = center_diff.mean()
        return dict(inner_radius_mean_relative_error=inner_radius_mean_relative_error, 
                    outer_radius_relative_error=outer_radius_mean_relative_error,
                    center_difference=center_diff_mean)
...
class MinimumCoveringEllipseTask(ShapefittingTask):
    def label(self, pointset):
        raw_pointcloud = pointset.numpy()
        C, r_maj, r_min, t = welzl(raw_pointcloud)
        return torch.tensor(np.concatenate((C, r_maj, r_min, t))).float()
    
    def compute_metrics(self):
        gt_center = datapoint.label[:, :-3]
        predicted_center = results[:, :-3]
        center_diff = torch.norm(gt_center - predicted_center, dim=1)
        center_diff_mean = center_diff.mean()
    ...
    def get_encoder_decoder(self, backbone: BackBone, loss_function: str='minimum_enclosing_ellipse_error', **kwargs):
        from nugets.models.encoder_decoders.shapefitting import MEEEncoderDecoder
        dataset_info = self.dataset_info()
        backbone_input_dim = backbone.get_input_dim()
        backbone_output_dim = backbone.get_output_dim()
        input_dim = dataset_info["dim"]
